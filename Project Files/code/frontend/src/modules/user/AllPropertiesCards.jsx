...
const AllPropertiesCards = ({ loggedIn }) => {
   ...
   const getAllProperties = async () => {
      try {
         const res = await axios.get('http://localhost:5000/api/user/getAllProperties');
   ...
   const handleBooking = async (status, propertyId, ownerId) => {
      try {
         await axios.post(`http://localhost:5000/api/user/bookinghandle/${propertyId}`, { userDetails, status, ownerId }, {
            headers: {
            ...
            {filteredProperties && filteredProperties.length > 0 ? (
               filteredProperties.map((property) => (
                  <Card border="dark" key={property._id} style={{ width: '18rem', marginLeft: 10 }}>
                     <Card.Body>
                        <Card.Title><img src={`http://localhost:5000${property.propertyImage[0].path}`} alt='photos' /></Card.Title>
                        <Card.Text>
